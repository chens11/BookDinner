...
#pragma mark - NSNotifcation Events
- (void)keyboardWillShow:(NSNotification*)notification{
    NSDictionary *userInfo = notification.userInfo;
    NSValue *rectValue = [userInfo objectForKey:UIKeyboardFrameEndUserInfoKey];
    CGRect rect = [rectValue CGRectValue];
    __weak UIView *view = self.alertView;
    [UIView animateWithDuration:0.4 animations:^{
        view.frame = CGRectMake(self.alertViewFrame.origin.x, self.frame.size.height - self.alertViewFrame.size.height - rect.size.height, self.alertViewFrame.size.width, self.alertViewFrame.size.height);
    }];
}
- (void)keyboardWillHide:(NSNotification*)notification{
    __weak UIView *view = self.alertView;
    [UIView animateWithDuration:0.2 animations:^{
        view.frame = self.alertViewFrame;
    }];
}
#pragma mark -
