...
- (void)reportFinished
{
	if (delegate && [delegate respondsToSelector:didFinishSelector]) {
        if (![self jsonValue]) {
            NSString *msg = [NSString stringWithFormat:@"action = %@\n resonseResult = %@",
                             [[self userInfo] valueForKey:@"user_info"],[self responseString]];
            UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"error log" message:msg delegate:nil cancelButtonTitle:@"I knows" otherButtonTitles:nil];
            [alter show];//chenzq-debug
        }
...
- (void)showError:(NSError *)errors{
    NSString *message = @"test";
    
    switch ([error code]) {
        case ASIRequestTimedOutErrorType:
            message = @"网络连接错误：网络连接超时！";
            break;
            
        case ASIUnhandledExceptionError:
            message = @"网络连接错误：网络连接未知错误！";
            break;
            
        case ASIConnectionFailureErrorType:
            message = @"网络连接错误：网络连接失败，请检查网络是否正常！";
            break;
            
        case ASIAuthenticationErrorType:
            message = [error localizedDescription]; // ＝@"A connection failure occurred: SSL problem (Possible causes may include a bad/expired/self-signed certificate, clock set to wrong date)" ，;
            break;
            
        case ASITooMuchRedirectionErrorType:
            message = @"网络连接错误：超过连接重试次数！"; // [errors localizedDescription] ＝@"The request failed because it redirected too many times;
            break;
            
        case ASIRequestCancelledErrorType:
            message = @"网络连接错误：网络请求被取消！"; // [errors localizedDescription]=@"The request was cancelled";
            break;
            
        case ASIUnableToCreateRequestErrorType:
            message = @"网络连接错误：未能创建服务请求！"; // [errors localizedDescription]=@"Unable to create request (bad url?)";
            break;
            
        case ASIInternalErrorWhileBuildingRequestType:
            message = @"网络连接错误：未能建立HTTP连接，请检查服务器地址设置参数是否正确！"; // [errors localizedDescription]＝Unable to start HTTP connection;
            break;
